...
//this is used to initialize the final 8 items
#define NGX_MODULE_V1_PADDING  0, 0, 0, 0, 0, 0, 0, 0

struct ngx_module_s {
    ngx_uint_t            ctx_index;	//模块内部的计数器,分类模块计数器
    ...
    void                 *ctx;			//该模块的上下文,所有模块有不同的上下文
    ngx_command_t        *commands;		//该模块的命令集,指向一个ngx_command_t结构数组
    ngx_uint_t            type;			//模块类别,为core/event/http/mail中的一种
...
struct ngx_conf_s {
    ...
    ngx_conf_file_t      *conf_file;		//conf文件信息
    ngx_log_t            *log;
